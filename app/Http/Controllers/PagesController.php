...
class PagesController extends Controller{
	...
	public function __construct(){
		...
		//return dd($experiences_routes->toArray());
		$this->phones_mex = [
		...
		View::share('experiences_routes',$experiences_routes);
	}
	...
	public function home(){
		...
		if(Agent::isMobile()){
			/*$offers = Offer::active()->main()->range()->with(['contents' => function($query){$query->where('lang_id', '=', $this->lang_id);}])
		...
		return view('pages.home',compact('page','offers','resort','destination','package'));
	}
	...
	public function destinationShow($destination){
		...
		if($destination){
			$resorts = Resort::active()->whereHas('destinations', function($q) use ($destination){
				...
				View::share('phones_customer',$this->phones_customer);
			return View('pages.destination', compact('destination','resorts')) ;
		}else{
			abort(404);
		}
	}
	...
	public function experienceShow($experience){
		...
		if($experience){
			...
			View::share('phones_car',$this->phones_car);
			View::share('phone_skype',$this->phone_skype_mex);
			return View('pages.experience', compact('experience','resorts')) ;
		}else{
			abort(404);
		}
	}
	...
	public function offerShow($offer){
		...
		if($offer){
			$travel_window = App\OfferTravelWindow::where('offer_id',$offer->id)->get();
