    	...
    	@if(count($all_offers) > 0) 
    		@foreach($all_offers as $key=>$offers)
    			<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 offer marginb50">
					<a href="{{url(App::getLocale().'/'.Lang::get('routes.offers').'/'.$offers->identifier)}}">
...
<div class="container">
	<div class="row">
		...
		<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 resort-small">
			<img class="img-responsive marco" src="{{asset('img/small/'.$resort->identifier.'-1.jpg')}}" alt="{{$resort->contents[0]->alt1}}">
			...
			<div class="marcoInferior marginb100">
				<a href="{{url(App::getLocale().'/'.Lang::get('routes.resorts').'/'.$resort->identifier)}}" class="offersHotels"><span>{{ $resort->name }}</span></a>
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 marginb50 text-center">
			<a href="{{url(App::getLocale().'/'.Lang::get('routes.offers'))}}" class="btn btn-danger btn-lg">@lang('messages.back_offers')</a>
		</div>
	</div>
</div>
@stop
